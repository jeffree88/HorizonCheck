### v7.9.33 beta candidate
- Adds `BETA_TESTING.md`, a focused closed-beta installation, smoke-test, high-risk scenario, and problem-reporting guide.
- Adds a structured GitHub beta bug-report form for consistent reproduction steps, UI information, screenshots, and release-health reports.
- Adds **Open Reports Folder** to Release Health plus `/hcheck health folder`, so normal testers can find exported reports without enabling Developer Mode.
- Extends release-health exports with the detailed deduplicated Diagnostics error list.
- Keeps Developer Mode off for every new profile and adds a release regression protecting that default.
- Marks tag-built GitHub releases as pre-releases during the closed beta.

### v7.9.28 highlights
- Makes Overview easier to scan with dynamic `COMPLETE`, `READY`, `RESET`, and `x LEFT` card wording while retaining exact counts underneath.
- Adds hover details and subtle session-change notes without adding progress bars or planner clutter.
- Aligns saved characters into a compact comparison table and keeps each row collapsed by default.
- Adds small visual separators between Current Character, Other Characters, and Shared Account.
- Keeps the performance-safe v7.9.27/v7.9.26 design: Overview does not trigger full inventory scans or additional background polling.
